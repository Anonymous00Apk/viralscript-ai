...
const PREFIXOS = { gemini: 'AIza', groq: 'gsk_', openrouter: 'sk-or-' };
...
// Converte erros do PROVEDOR em erro com status HTTP adequado, para que o
// navegador do usuário receba o código correto (401/402/429) e não um 500 genérico.
function erroHTTP(provedor, res) {
  let erro;
  ...
  erro.status = res.status; // repassa o status real ao cliente (compatível com o tratamento do index.html)
  return erro;
}
...
async function chamarGemini(apiKey, prompt) {
  ...
  if (!res.ok) throw erroHTTP('Gemini', res);
...
async function chamarGroq(apiKey, prompt) {
  ...
  if (!res.ok) throw erroHTTP('Groq', res);
...
async function chamarOpenRouter(apiKey, prompt) {
  ...
  if (!res.ok) throw erroHTTP('OpenRouter', res);
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!apiKey.startsWith(PREFIXOS[provedor])) {
      return { statusCode: 401, headers, body: JSON.stringify({ erro: 'A chave informada não parece ser do ' + NOMES[provedor] + ' (esperado prefixo "' + PREFIXOS[provedor] + '"). Confira em "Configurar Chave".' }) };
    }
    ...
    return { statusCode: status, headers, body: JSON.stringify({ erro: err.message || 'Erro interno do servidor.' }) };
  }
};
