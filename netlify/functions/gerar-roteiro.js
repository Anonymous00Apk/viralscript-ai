...
// 🆕 V3.4: CASCATA DE MODELOS+COSTURA DE PENSAMENTO, ordenada do MAIS RÁPIDO
// para o mais lento. Contexto técnico:
//  - gemini-2.5-flash: aposentado pelo Google para chaves novas (404);
//  - gemini-3.6-flash: modelo "thinking" — raciocina >25s (estoura o teto
//    da Netlify) e REJEITA thinkingBudget 0 (400 invalid argument);
//  - gemini-flash-latest: ALIAS oficial do Google que sempre aponta para o
//    flash atual recomendado — blindagem estrutural contra aposentadorias;
//  - gemini-3.6-flash-lite: variante rápida do sucessor (não "pensa");
//  - tentativa final do 3.6-flash com thinkingBudget MODERADO (512): pensa
//    um pouco — pode caber nos 25s (é a última chance, por isso no fim).
// A cascata tenta cada combinação em ordem; falhas de MODELO (400/404/
// timeout) avançam para a próxima; falhas de CHAVE/conta (401/402/403/429)
// interrompem imediatamente. O usuário final não configura NADA.
const CASCATA_GEMINI = [
  { modelo: process.env.AI_GEMINI_MODEL, budget: 0 },      // 1º: env var (se definida)
  { modelo: 'gemini-flash-latest', budget: 0 },             // 2º: alias oficial atual
  { modelo: 'gemini-3.6-flash-lite', budget: 0 },           // 3º: lite rápido
  { modelo: 'gemini-2.0-flash', budget: 0 },                // 4º: legado rápido
  { modelo: 'gemini-3.6-flash', budget: 512 },              // 5º: thinking curto
  { modelo: 'gemini-3.6-flash', budget: 0 },                // 6º: reteste budget 0
  { modelo: 'gemini-2.5-flash', budget: 0 }                 // 7º: legado (404 p/ chaves novas)
...
async function fetchComTimeout(url, options) {
  ...
  try {
    ...
    if (err.name === 'AbortError') {
      const erro = new Error('O modelo Gemini demorou demais (modelo "thinking" estoura o limite da hospedagem).');
...
// 🆕 V3.4: CASCATA — tenta cada combinação modelo+budget em ordem de
// velocidade. Regras:
...
//    (tentar outro modelo não resolveria);
...
async function chamarGemini(apiKey, prompt) {
  ...
  const erro = new Error(
    'Nenhuma combinação de modelo Gemini funcionou com sua chave (testados: ' + tentados.join('; ') + ').' +
    (ultimoErro ? ' Última falha: ' + ultimoErro.message + (ultimoErro.detalhe ? ' — ' + ultimoErro.detalhe : '') : '') +
    ' Sugestão imediata: selecione Groq ou OpenRouter no app (funcionam normalmente) — ou aguarde o Google liberar os modelos para novas chaves.'
  );
  erro.status = 502;
  throw erro;
}
