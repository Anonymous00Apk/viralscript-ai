...
const PROMPT_TEMPLATE = String.raw`Você é o Principal Engenheiro de Conteúdo Viral e Especialista em Copywriting de Elite para Monetização Multiplataforma (TikTok, Kwai, YouTube Shorts, YouTube Normal/Longo e Facebook Video). Sua missão é criar roteiros cirúrgicos de alta conversão, otimizados para maximizar o tempo de exibição (Watch Time) e a receita em qualquer plataforma.
...
- Se estiver vazia, "-", "auto" ou "automático", use a duração padrão da plataforma: TikTok/Kwai = 30-45s · Shorts = 30-60s · YouTube Longo = 8-12 minutos · Facebook = 60-90s.
...
3. CALL TO ACTION (CTA) DE ALTA MONETIZAÇÃO:
   - CTA focada na metrificação principal da plataforma {plataforma} (comentários polêmicos, compartilhamento ou inscrição/seguidor).
...
// 🆕 V5: TIMEOUT 20s (era 25s). Motivo: com a cascata de modelos, cada
// tentativa precisa deixar margem para a próxima dentro do TETO de 26s da
// Netlify gratuita. 20s por tentativa + resgate = uso saudável do budget.
...
// 🆕 V3.3: CASCATA DE MODELOS. O Google aposenta modelos e lança sucessores
// com frequência (gemini-2.5-flash bloqueado p/ novos usuários; 3.6-flash
// "pensa" >25s e não aceita thinkingBudget 0). A cascata tenta em ordem:
//   1º env AI_GEMINI_MODEL (se definida no Netlify)
//   2º gemini-flash-latest        (alias oficial anti-aposentadoria)
//   3º gemini-3.6-flash-lite      (variante rápida)
//   4º gemini-2.0-flash           (legado rápido)
//   5º gemini-3.6-flash (budget 512 — "pensamento" curto)
//   6º gemini-3.6-flash (budget 0)
//   7º gemini-2.5-flash           (legado)
// Falhas 400/404 (modelo incompatível/ínexistente) avançam p/ o próximo;
// falhas de CHAVE/conta (401/402/403/429) interrompem imediatamente.
const CASCATA_GEMINI = [
  process.env.AI_GEMINI_MODEL,
  'gemini-flash-latest',
  'gemini-3.6-flash-lite',
  'gemini-2.0-flash',
  { modelo: 'gemini-3.6-flash', budget: 512 },
  { modelo: 'gemini-3.6-flash', budget: 0 },
  'gemini-2.5-flash'
].map(c => (typeof c === 'string' ? { modelo: c, budget: 0 } : c))
 .filter(c => c.modelo && c.modelo.trim());
...
async function fetchComTimeout(url, options) {
  ...
  try {
    ...
    if (err.name === 'AbortError') {
      const erro = new Error('O modelo Gemini demorou demais para responder (modelo "thinking" estoura o limite da hospedagem).');
      erro.abortGemini = true; // sinaliza à cascata para tentar o próximo candidato
      throw erro;
    }
...
// Chamada individual a UM modelo com UM budget de pensamento (usada pela
// cascata). A chave do Gemini é enviada via HEADER x-goog-api-key (método
// documentado pelo Google, compatível com chaves "AIza..." e "AQ....").
async function chamarGeminiTentativa(apiKey, prompt, modelo, thinkingBudget) {
...
// V3.3: CASCATA — tenta cada combinação modelo+budget em ordem de
// velocidade. Regras:
//  - 400/404/timeout (modelo incompatível/ínexistente/"thinking" lento)
//    → PRÓXIMO candidato;
//  - 401/402/403/429 (problema de CHAVE/conta do usuário) → LANÇA na hora;
//  - sucesso → retorna imediatamente.
// Se todos falharem: erro consolidado com o diagnóstico de cada tentativa.
async function chamarGemini(apiKey, prompt) {
  const tentados = [];
  let ultimoErro = null;

  for (const candidato of CASCATA_GEMINI) {
    const rotulo = candidato.modelo + ' (budget ' + candidato.budget + ')';
    if (tentados.includes(rotulo)) continue;
    tentados.push(rotulo);
    try {
      return await chamarGeminiTentativa(apiKey, prompt, candidato.modelo, candidato.budget);
    } catch (err) {
      ultimoErro = err;
      // Problema de chave/conta: não é do modelo — lança imediatamente
      if (err.status && [401, 402, 403, 429].includes(err.status)) throw err;
      // Modelo incompatível/ínexistente/"thinking" lento → próximo candidato
      continue;
    }
  }

  // Todas as combinações falharam: erro consolidado com diagnóstico
  const erro = new Error(
    'Nenhum modelo do Gemini funcionou com sua chave (testados: ' + tentados.join('; ') + ').' +
    (ultimoErro ? ' Última falha: ' + ultimoErro.message + (ultimoErro.detalhe ? ' — ' + ultimoErro.detalhe : '') : '') +
    ' Sugestão imediata: selecione Groq ou OpenRouter no app (funcionam normalmente).'
  );
  erro.status = 502;
  throw erro;
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // provedor (401/402/429); demais falhas permanecem 500 (502 p/ cascata
    // de modelos esgotada).
